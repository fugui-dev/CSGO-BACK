package com.ruoyi.thirdparty.zhaocaipay.controller;
...
@Slf4j
@RestController
@RequestMapping("api/zcPay")
public class ZCPayController extends BaseController {

    @Autowired
    TtUserService userService;

    @Autowired
    ZhaoCaiPayService zhaoCaiPayService;
    ...
    public R<UnifyPayPreOrderVO> preOrder(HttpServletRequest request, @RequestBody @Validated CreateOrderParam param) {
        ...
        return zhaoCaiPayService.preOrder(param, user, request);
    }
    ...
    public String notify(HttpServletRequest request) {
        ...
        //处理回调
        try {
            return zhaoCaiPayService.callback(request);
