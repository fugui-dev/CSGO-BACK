package com.ruoyi.thirdparty.decsgopay.csgopay.service;
...
import com.ruoyi.thirdparty.decsgopay.csgopay.sdk.CSPaySDK;
...
@Service
@Slf4j
public class CSPayService {

    @Autowired
    CSPayConfig payConfig;
...
//        Asserts.notBlank(param.getPayType(), "支付方式不能为空！");
        ...
        //校验金额是否符合支付规则
        unifyPayPerOrderProcess.checkPayConfigRule(totalAmount, user, "csPay");

        //下单
        CSPaySDK paySDK = new CSPaySDK(payConfig);
        ...
        if (dataObj != null){
            ...
            //存储订单
            unifyPayPerOrderProcess.savePayOrder(orderNo, outTradeNo, user, param, PayType.CS_PAY, totalAmount, url);
