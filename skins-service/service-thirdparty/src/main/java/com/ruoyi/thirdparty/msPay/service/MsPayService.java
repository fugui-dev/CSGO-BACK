package com.ruoyi.thirdparty.msPay.service;
...
import com.ruoyi.thirdparty.msPay.sdk.MsPaySDK;
import com.ruoyi.thirdparty.msPay.sdk.util.KeyUtils;
import com.ruoyi.thirdparty.msPay.sdk.util.NotifyCheck;
...
@Service
@Slf4j
public class MsPayService {

    @Autowired
    MsPayConfig payConfig;
    ...
    public R<UnifyPayPreOrderVO> preOrder(CreateOrderParam param, TtUser user, HttpServletRequest request) {
        ...
        //校验金额是否符合支付规则
        unifyPayPerOrderProcess.checkPayConfigRule(totalAmount, user, "msPay");

        //下单
        MsPaySDK paySDK = new MsPaySDK(payConfig);
//        String orderNo = IdUtils.fastSimpleUUID(); //开箱系统订单号
        String orderNo = "mpay" + KeyUtils.getOutTradeNo() + "_";
        ...
        ResponseDTO rspDTO = paySDK.preOrder(totalAmount, user.getUserId().toString(), orderNo);

        //判断结果，响应前端
        if (rspDTO != null){

            String url = rspDTO.getCodeUrl();
            String outTradeNo = rspDTO.getOrderNo(); //外部订单号

            //存储订单
            unifyPayPerOrderProcess.savePayOrder(orderNo, outTradeNo, user, param, PayType.MS_PAY, totalAmount, url);
    ...
    public String callback(HttpServletRequest request) throws Exception {
        ...
        log.info("收到民生支付回调==>【{}】", paramMap);

        //1验签
        Boolean check = new NotifyCheck().isCheck(paramMap.get("biz_content"), paramMap.get("sign"));
        log.info("民生支付验签结果==>【{}】", check);
        Assert.isTrue("T".equals(JSONObject.parseObject(paramMap.get("biz_content")).get("is_success")));

        //2向支付方查询订单号，如果查询可以不验签
        String orderNo = JSONObject.parseObject(paramMap.get("biz_content")).get("out_trade_no").toString();
//        boolean payFlag = paySDK.queryOrder(orderNo);
//        Assert.isTrue(payFlag, "未查询到支付成功的信息！");

        //3调用统一处理流程
        String rstr = unifyCallbackProcess.notifyProcess(orderNo);
